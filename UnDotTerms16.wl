...
BeginPackage["Disruptioneering`UnDotTerms`"];
...
UnDotTerms[term_, undotterms_, scalars_, vectors_, tensors_, closurevar_, \
...
gradform_, identitymatrixform_, normalvecform_, Uform_, slowIV_, fastIV_, option_, option2_] := 
    procedure[term, undotterms, scalars, vectors, tensors, closurevar, graddenoters, \
      ...
      identitymatrixform, normalvecform, Uform, slowIV, fastIV, option, option2];


procedure[term_, undotterms_, scalars_, vectors_, tensors_, closurevar_, graddenoters_, \
...
identitymatrixform_, normalvecform_, Uform_, slowIV_, fastIV_, option_, option2_] :=
    Block[{solution, solutiondummy, uvecdependency, newvectors, i, isubin, ii, \
    levelcountdummy = 0, lookforinfo, subininfo, symbol, flag = 0, scalarTermsToTry, \
    scalarTermsToTryInv},
        
        newvectors = vectors;
        
        solutiondummy = term;
        For[isubin = 1, isubin <= Length[undotterms], isubin++,
            lookforinfo = getterminfo[Expand[undotterms[[isubin]]], {}, scalars, \
              ...
              slowIV, fastIV, levelcountdummy, {}];
            
            solution = main[Expand[solutiondummy], Expand[undotterms[[isubin]]], \
              ...
              identitymatrixform, normalvecform, Uform, slowIV, fastIV, 1, option2];
            
            (*This part really only works if the function is given one equation*)
            If[solution == solutiondummy,
                Print["UnDotTerms: procedure: looking to scale equation by scalars for undotting..."];
                
                flag = 0;
                For[i = 1, i <= Length[lookforinfo], i++,
                    For[ii = 1, ii <= Length[lookforinfo[[i, 3]]], ii++,
                        If[lookforinfo[[i, 2, ii]] == 0 && lookforinfo[[i, 3, ii]] == 0,
                            solution = main[Expand[solutiondummy * lookforinfo[[i, 1, ii]]], \
                              Expand[undotterms[[isubin]]], scalars, newvectors, tensors, \
                              ...
                              identitymatrixform, normalvecform, Uform, slowIV, fastIV, 1, \
                              option2];
                            If[solution == Expand[solutiondummy * lookforinfo[[i, 1, ii]]],
                                ,
                                flag = 1;
                                Break[];
                                ,
                ...
                If[flag == 1,
                    Print["UnDotTerms: procedure: successfully undotted during scalar analysis."];
                    ,
                    Print["UnDotTerms: procedure: failed to undot during scalar analysis."];
                ];
            ];
            
            solutiondummy = solution;
        ];
...
main[term_, undotterms_, scalars_, vectors_, tensors_, closurevar_, graddenoters_, \
divdenoters_, avgdenoters_, derivativedenoters_, dotdenoters_, tensorproductdenoters_, doubledotdenoters_, pietrzykflipdenoters_, divform_, gradform_, \
identitymatrixform_, normalvecform_, Uform_, slowIV_, fastIV_, option_, option2_] :=
    Block[{solution},
        
        If[Head[term] == List,
            solution = caseListmain[term, undotterms, scalars, vectors, tensors, \
              closurevar, graddenoters, divdenoters, avgdenoters, derivativedenoters, dotdenoters, \
              tensorproductdenoters, doubledotdenoters, pietrzykflipdenoters, divform, gradform, identitymatrixform, normalvecform, \
              Uform, slowIV, fastIV, option, option2];
            ,
            solution = caseNonListmain[term, undotterms, scalars, vectors, tensors, \
              closurevar, graddenoters, divdenoters, avgdenoters, derivativedenoters, dotdenoters, \
              tensorproductdenoters, doubledotdenoters, pietrzykflipdenoters, divform, gradform, identitymatrixform, normalvecform, \
              Uform, slowIV, fastIV, option, option2];
            ,
...
caseListmain[term_, undotterms_, scalars_, vectors_, tensors_, closurevar_, graddenoters_, \
divdenoters_, avgdenoters_, derivativedenoters_, dotdenoters_, tensorproductdenoters_, doubledotdenoters_, pietrzykflipdenoters_, divform_, gradform_, \
identitymatrixform_, normalvecform_, Uform_, slowIV_, fastIV_, option_, option2_] :=
    Block[{i, solution = ConstantArray[0, Length[Expand[term]]], toplayer = option, chosen = 0,
    successfullyfound, ilookfor, lookfor, lookforinfo, levelcountdummy = 0, newscalars, \
    ...
    successsolutionRankindex, icheckmult, ichecksolution, isubin, isuccesssolution, \
    itermsolution, submults, subinsolution, suboutsolution, solutiondummy, ichecksoleq, \
    ichecksolsec, undottedparts, gotit, ilookforterm, ieqsection2, itermmults2, flagnext, \
    dummysuccessmultsindex, successfullyfoundneg, successfullyfoundneg2, \
    savedlookforinfoRank, sectiontoplace, Nlookfor, potentialmultipliedpartssolutionRank, \
    successfullyfound2Rank, successsolutionRank, Nterms, isections, Nsubs, skip, \
    successfullyfoundneg2Rank, newsuccessmultsindex, icheckaddedterm, icombo, ichecksection, \
    itermcases, icheckcases, newcombo, dummysuccesssolutionindex, successsolutionindex, \
    newsuccesssolutionindex, finalindices, expandedlookfor, inegone, inegone0, inegone2, \
    flagnegone},
        ...
        For[i = 1, i <= Length[expandedterm], i++,
            If[toplayer == 1,
                ...
                If[Head[lookfor] == Plus,
                    Nlookfor = Length[lookfor];
                    ,
                    Nlookfor = 1;
                    ,
                    Nlookfor = 1;
                ];
                successfullyfound = ConstantArray[0, Nlookfor];
                successfullyfoundneg = ConstantArray[0, Nlookfor];
                expandedtermfound = ConstantArray[0, Nlookfor];
                potentialmultipliedparts = ConstantArray[0, Nlookfor];
                potentialmultipliedpartssolution = ConstantArray[0, Nlookfor];
                potentialmultipliedpartssolutionRank = ConstantArray[0, Nlookfor];
                savedlookforinfoRank = ConstantArray[0, Nlookfor];
                For[ilookfor = 1, ilookfor <= Nlookfor, ilookfor++,
                    ...
                    savedlookforinfoRank[[ilookfor]] = lookforinfo[[1, 2, 1]];
                    
                    
                    looking = 1;
                    If[lookforinfo[[1, 1, 1]] == -1,
                        
                        successfullyfound[[ilookfor]] = main[expandedterm[[i]], \
                          {{{{option2}, {lookforinfo[[1, 2, 1]]}, {lookforinfo[[1, 3, 1]]}}}, \
                          looking}, newscalars, newvectors, newtensors, closurevar, graddenoters, \
                          ...
                          gradform, identitymatrixform, normalvecform, Uform, slowIV, fastIV, 0, \
                          option2];
                        successfullyfound[[ilookfor]] = Expand[successfullyfound[[ilookfor]]];
                        
                        For[inegone = 1, inegone <= Length[successfullyfound[[ilookfor]]], \
                          inegone++,
                            For[inegone0 = 1, inegone0 <= Length[successfullyfound[[ilookfor, \
                             inegone]]], inegone0++,
                                flagnegone = 0;
                                For[inegone2 = 1, inegone2 <= Length[successfullyfound[[ \
                                  ilookfor, inegone, inegone0, 1]]], inegone2++,
                                    If[successfullyfound[[ilookfor, inegone, inegone0, 1, \
                                      inegone2]] == -1,
                                        flagnegone = 1;
                                        successfullyfound[[ilookfor, inegone, inegone0, 1, \
                                          inegone2]] = option2;
                                    ];
                                ];
                                If[flagnegone == 0,
                                    For[inegone2 = 1, inegone2 <= 2, inegone2++,
                                        successfullyfound[[ilookfor, inegone, inegone0, 1]] = \
                                          Join[successfullyfound[[ilookfor, inegone, inegone0, \
                                          1]], {option2}];
                                        successfullyfound[[ilookfor, inegone, inegone0, 2]] = \
                                          Join[successfullyfound[[ilookfor, inegone, inegone0, \
                                          2]], {0}];
                                        successfullyfound[[ilookfor, inegone, inegone0, 3]] = \
                                          Join[successfullyfound[[ilookfor, inegone, inegone0, \
                                          3]], {0}];
                                    ];
                                ];
                        ...
                        successfullyfoundneg[[ilookfor]] = main[-expandedterm[[i]], \
                          {{{{option2}, {lookforinfo[[1, 2, 1]]}, {lookforinfo[[1, 3, 1]]}}}, \
                          looking}, newscalars, newvectors, newtensors, closurevar, \
                          graddenoters, divdenoters, avgdenoters, derivativedenoters, \
                          dotdenoters, tensorproductdenoters, doubledotdenoters, \
                          pietrzykflipdenoters, divform, gradform, identitymatrixform, \
                          normalvecform, Uform, slowIV, fastIV, 0, option2];
                        successfullyfoundneg[[ilookfor]] = Expand[successfullyfoundneg[[ilookfor]]];
                        
                        For[inegone = 1, inegone <= Length[successfullyfoundneg[[ilookfor]]], \
                          inegone++,
                            For[inegone0 = 1, inegone0 <= Length[successfullyfoundneg[[ilookfor, \
                             inegone]]], inegone0++,
                                flagnegone = 0;
                                For[inegone2 = 1, inegone2 <= Length[successfullyfoundneg[[ \
                                  ilookfor, inegone, inegone0, 1]]], inegone2++,
                                    If[successfullyfoundneg[[ilookfor, inegone, inegone0, 1, \
                                      inegone2]] == -1,
                                        flagnegone = 1;
                                        successfullyfoundneg[[ilookfor, inegone, inegone0, 1, \
                                          inegone2]] = option2;
                                    ];
                                ];
                                If[flagnegone == 0,
                                    For[inegone2 = 1, inegone2 <= 2, inegone2++,
                                        successfullyfoundneg[[ilookfor, inegone, inegone0, 1]] = \
                                          Join[successfullyfoundneg[[ilookfor, inegone, inegone0, \
                                          1]], {option2}];
                                        successfullyfoundneg[[ilookfor, inegone, inegone0, 2]] = \
                                          Join[successfullyfoundneg[[ilookfor, inegone, inegone0, \
                                          2]], {0}];
                                        successfullyfoundneg[[ilookfor, inegone, inegone0, 3]] = \
                                          Join[successfullyfoundneg[[ilookfor, inegone, inegone0, \
                                          3]], {0}];
                                    ];
                                ];
                        ...
                        successfullyfound[[ilookfor]] = main[expandedterm[[i]], \
                          {lookforinfo, looking}, newscalars, newvectors, newtensors, \
                          closurevar, graddenoters, divdenoters, avgdenoters, derivativedenoters, dotdenoters, \
                          tensorproductdenoters, doubledotdenoters, pietrzykflipdenoters, divform, gradform, identitymatrixform, \
                          normalvecform, Uform, slowIV, fastIV, 0, option2];
                        successfullyfound[[ilookfor]] = Expand[successfullyfound[[ilookfor]]];
                        successfullyfoundneg[[ilookfor]] = main[-expandedterm[[i]], \
                          {lookforinfo, looking}, newscalars, newvectors, newtensors, \
                    ...
                    potentialmultipliedpartssolution[[ilookfor]] = ConstantArray[0, Length[ \
                      successfullyfound[[ilookfor]]]];
                    potentialmultipliedpartssolutionRank[[ilookfor]] = ConstantArray[0, \
                    ...
                    For[ieqsection = 1, ieqsection <= Length[ \
                      successfullyfound[[ilookfor]]], ieqsection++,
                        ...
                        potentialmultipliedpartssolutionRank[[ilookfor, ieqsection]] = {};
                        ...
                        For[iaddedterms = 1, iaddedterms <= Length[ \
                          successfullyfound[[ilookfor, ieqsection]]], iaddedterms++,
                            ...
                            potentialmultipliedpartssolution[[ilookfor, ieqsection]] = \
                              ...
                              ieqsection]], {{}}];
                            potentialmultipliedpartssolutionRank[[ilookfor, ieqsection]] = \
                              Join[potentialmultipliedpartssolutionRank[[ilookfor, \
                              ieqsection]], {{}}];
                            successfullyfound2 = successfullyfound[[ilookfor, ieqsection, \
                              iaddedterms, 1]];
                            successfullyfound2Rank = successfullyfound[[ilookfor, ieqsection, \
                              iaddedterms, 2]];
                              ...
                              ioperator++,
                                ...
                                If[successfullyfound2[[ioperator]] == \
                                  dummysuccessfullyfound2,
                                    ...
                                    potentialmultipliedpartssolution[[ilookfor, ieqsection, -1]] = \
                                      Join[potentialmultipliedpartssolution[[ilookfor, \
                                      ieqsection, -1]], {successfullyfound2[[ioperator]]}];
                                    potentialmultipliedpartssolutionRank[[ilookfor, ieqsection, -1]] = \
                                      Join[potentialmultipliedpartssolutionRank[[ilookfor, \
                                      ieqsection, -1]], {successfullyfound2Rank[[ioperator]]}];
                                    If[Nlookfor > 1,
                        ...
                        If[Flatten[potentialmultipliedparts[[ilookfor]]] == {} && \
                          Flatten[potentialmultipliedpartssolution[[ilookfor]]] == {},
                            ...
                            For[iaddedterms = 1, iaddedterms <= Length[ \
                              successfullyfoundneg[[ilookfor, ieqsection]]], iaddedterms++,
                                ...
                                successfullyfoundneg2Rank = successfullyfoundneg[[ilookfor, \
                                  ieqsection, iaddedterms, 2]];
                                    ...
                                    If[successfullyfoundneg2[[ioperator]] == \
                                      dummysuccessfullyfound2,
                                        ...
                                        potentialmultipliedpartssolution[[ilookfor, \
                                          ...
                                          ioperator]]}];
                                        potentialmultipliedpartssolutionRank[[ilookfor, \
                                          ieqsection, -1]] = Join[ \
                                          potentialmultipliedpartssolutionRank[[ilookfor, \
                                          ieqsection, -1]], {successfullyfoundneg2Rank[[ \
                                          ioperator]]}];
                                        ,
                                        If[Nlookfor > 1,
                                            multipliedparts = -listProduct[Delete[ \
                ...
                If[Equal @@ savedlookforinfoRank,
                    savedlookforinfoRank = savedlookforinfoRank[[1]];
                    ,
                    Print["CRITICAL ERROR: caseListmain: lookfor terms are not the same rank. The term is: ", lookfor];
                    Return[Null];
                ];
                
                looking = 1;
                expandedtermfound = main[expandedterm[[i]], \
                  {{{{option2}, {0}, {0}}}, looking}, newscalars, newvectors, newtensors, \
                  closurevar, graddenoters, divdenoters, avgdenoters, derivativedenoters, dotdenoters, \
                  tensorproductdenoters, doubledotdenoters, pietrzykflipdenoters, divform, gradform, identitymatrixform, \
                  normalvecform, Uform, slowIV, fastIV, 0, option2];
                expandedlookfor = main[{{lookfor}}, \
                  {{{{option2}, {0}, {0}}}, looking}, newscalars, newvectors, newtensors, \
                  closurevar, graddenoters, divdenoters, avgdenoters, derivativedenoters, dotdenoters, \
                  tensorproductdenoters, doubledotdenoters, pietrzykflipdenoters, divform, gradform, identitymatrixform, \
                  normalvecform, Uform, slowIV, fastIV, 0, option2][[1, 1]];
                looking = 0;
                
                
                successmults = potentialmultipliedparts[[1]];
                successmultsindex = {};
                dummysuccessmultsindex = {};
                successsolution = potentialmultipliedpartssolution[[1]];
                successsolutionindex = {};
                dummysuccesssolutionindex = {};
                successsolutionRank = potentialmultipliedpartssolutionRank[[1]];
                successsolutionRankindex = {};
                
                sectiontoplace = {};
                
                (*Check for similar multiples*)
                For[ilookfor = 1, ilookfor <= Length[potentialmultipliedparts], \
                  ilookfor++,
                    For[ieqsection = 1, ieqsection <= Length[successmults], ieqsection++,
                        For[iaddedterms = 1, iaddedterms <= Length[successmults[[ \
                          ieqsection]]], iaddedterms++,
                            successmults[[ieqsection, iaddedterms]] = Intersection[ \
                              successmults[[ieqsection, iaddedterms]], Flatten[ \
                              potentialmultipliedparts[[ilookfor]]]];
                        ];
                    ];
                ];
                If[successmults != {},
                    For[ieqsection = 1, ieqsection <= Length[successmults], ieqsection++,
                        For[iaddedterms = 1, iaddedterms <= Length[successmults[[ieqsection]]], iaddedterms++,
                            For[itermcases = 1, itermcases <= Length[successmults[[ieqsection, iaddedterms]]], itermcases++,
                                dummysuccessmultsindex = Join[dummysuccessmultsindex, {{{ieqsection, iaddedterms, itermcases}}}];
                                For[ilookfor = 2, ilookfor <= Length[potentialmultipliedparts], ilookfor++,
                                    newsuccessmultsindex = {};
                                    For[icombo = 1, icombo <= Length[dummysuccessmultsindex], icombo++,
                                        For[ichecksection = 1, ichecksection <= Length[potentialmultipliedparts[[ilookfor]]], ichecksection++,
                                            For[icheckaddedterm = 1, icheckaddedterm <= Length[potentialmultipliedparts[[ilookfor, ichecksection]]], icheckaddedterm++,
                                                If[Intersection[dummysuccessmultsindex[[icombo, ;;, 1 ;; 2]], {{ichecksection, icheckaddedterm}}] == {},
                                                    For[icheckcases = 1, icheckcases <= Length[potentialmultipliedparts[[ilookfor, ichecksection, icheckaddedterm]]], icheckcases++,
                                                        If[successmults[[ieqsection, iaddedterms, itermcases]] == potentialmultipliedparts[[ilookfor, ichecksection, icheckaddedterm, icheckcases]],
                                                            newcombo = Join[dummysuccessmultsindex[[icombo]], {{ichecksection, icheckaddedterm, icheckcases}}];
                ...
                For[ilookfor = 1, ilookfor <= Length[potentialmultipliedpartssolution], \
                  ilookfor++,
                    For[ieqsection = 1, ieqsection <= Length[successsolution], ieqsection++,
                        For[iaddedterms = 1, iaddedterms <= Length[successsolution[[ \
                          ieqsection]]], iaddedterms++,
                            successsolution[[ieqsection, iaddedterms]] = Intersection[ \
                              successsolution[[ieqsection, iaddedterms]], Flatten[ \
                              potentialmultipliedpartssolution[[ilookfor]]]];
                        ];
                    ];
                ];
                If[successsolution != {},
                    For[ieqsection = 1, ieqsection <= Length[successsolution], ieqsection++,
                        For[iaddedterms = 1, iaddedterms <= Length[successsolution[[ieqsection]]], iaddedterms++,
                            For[itermcases = 1, itermcases <= Length[successsolution[[ieqsection, iaddedterms]]], itermcases++,
                                dummysuccesssolutionindex = Join[dummysuccesssolutionindex, {{{ieqsection, iaddedterms, itermcases}}}];
                                For[ilookfor = 2, ilookfor <= Length[potentialmultipliedpartssolution], ilookfor++,
                                    newsuccesssolutionindex = {};
                                    For[icombo = 1, icombo <= Length[dummysuccesssolutionindex], icombo++,
                                        For[ichecksection = 1, ichecksection <= Length[potentialmultipliedpartssolution[[ilookfor]]], ichecksection++,
                                            For[icheckaddedterm = 1, icheckaddedterm <= Length[potentialmultipliedpartssolution[[ilookfor, ichecksection]]], icheckaddedterm++,
                                                If[Intersection[dummysuccesssolutionindex[[icombo, ;;, 1 ;; 2]], {{ichecksection, icheckaddedterm}}] == {},
                                                    For[icheckcases = 1, icheckcases <= Length[potentialmultipliedpartssolution[[ilookfor, ichecksection, icheckaddedterm]]], icheckcases++,
                                                        If[successsolution[[ieqsection, iaddedterms, itermcases]] == potentialmultipliedpartssolution[[ilookfor, ichecksection, icheckaddedterm, icheckcases]],
                                                            newcombo = Join[dummysuccesssolutionindex[[icombo]], {{ichecksection, icheckaddedterm, icheckcases}}];
                ...
                successsolutionRankindex = successsolutionindex;
                ...
                For[itermmults = 1, itermmults <= Length[successmultsindex], itermmults++,
                    If[Intersection[Flatten[finalindices[[;;, ;;, 1 ;; 2]], 1], \
                      successmultsindex[[itermmults, ;;, 1 ;; 2]]] == {},
                        For[itermsolution = 1, itermsolution <= Length[successsolutionindex], \
                          itermsolution++,
                            If[Intersection[Flatten[finalindices[[;;, ;;, 1 ;; 2]], 1], \
                              successsolutionindex[[itermsolution, ;;, 1 ;; 2]]] == {},
                                If[successmultsindex[[itermmults]] == \
                                  successsolutionindex[[itermsolution]],
                                    If[finalindices == {{{0, 0, 0}}},
                ...
                If[finalindices == {{{0, 0, 0}}},
                    Print["Exiting Procedure: caseListmain: Unable to undot/divide from equation."];
                    Return[term];
                ];
                ...
                Nterms = 0;
                For[ieqsection = 1, ieqsection <= Length[expandedtermfound], ieqsection++,
                    chosen = 0;
                    If[expandedtermfound[[ieqsection]] == {{{0}, {0}, {0}}},
                        chosen = 1;
                    ];
                    If[chosen == 0,
                        Nterms += Length[expandedtermfound[[ieqsection]]];
                    ];
                ];
                Nsubs = Length[finalindices] * Length[expandedlookfor];
                If[Nsubs != Nterms,
                    Print["Exiting Procedure: caseListmain: Unable to undot/divide from all terms in equation."];
                    Return[term];
                ];
                ...
                For[isubin = 1, isubin <= Length[finalindices], isubin++,
                    
                    sectiontoplace = Min[finalindices[[isubin, ;;, 1]]];
                    
                    If[Nlookfor > 1,
                        For[ilookfor = 1, ilookfor <= Nlookfor, ilookfor++,
                            solution[[i, finalindices[[isubin, ilookfor, 1]]]] = \
                              Expand[solution[[i, finalindices[[isubin, ilookfor, 1]]]] - \
                              potentialmultipliedpartssolution[[ilookfor, \
                              finalindices[[isubin, ilookfor, 1]], finalindices[[ \
                              isubin, ilookfor, 2]], finalindices[[ \
                              isubin, ilookfor, 3]]]] * potentialmultipliedparts[[ilookfor, \
                              finalindices[[isubin, ilookfor, 1]], finalindices[[ \
                              isubin, ilookfor, 2]], finalindices[[ \
                              isubin, ilookfor, 3]]]]] /. option2 -> lookfor[[ilookfor]];
                        ];
                        ,
                        solution[[i, finalindices[[isubin, 1, 1]]]] = \
                          Expand[solution[[i, finalindices[[isubin, 1, 1]]]] - \
                          potentialmultipliedpartssolution[[1, \
                          finalindices[[isubin, 1, 1]], finalindices[[ \
                          isubin, 1, 2]], finalindices[[ \
                          isubin, 1, 3]]]] * potentialmultipliedparts[[1, \
                          finalindices[[isubin, 1, 1]], finalindices[[ \
                          isubin, 1, 2]], finalindices[[ \
                          isubin, 1, 3]]]]] /. option2 -> lookfor;
                    ];
                    
                    
                    chosen = 0;
                    If[savedlookforinfoRank == 0,
                        chosen = 1;
                        solution[[i, sectiontoplace]] = \
                          Expand[solution[[i, sectiontoplace]] + \
                          potentialmultipliedparts[[1, finalindices[[isubin, 1, 1]], \
                          finalindices[[isubin, 1, 2]], \
                          finalindices[[isubin, 1, 3]]]]];
                    ];
                    
                    If[savedlookforinfoRank == 1,
                        chosen = 1;
                        undottedparts = Undotter[potentialmultipliedpartssolution[[1, \
                          finalindices[[isubin, 1, 1]], \
                          finalindices[[isubin, 1, 2]], \
                          finalindices[[isubin, 1, 3]]]], \
                          option2, dotdenoters, doubledotdenoters, identitymatrixform];
                        If[undottedparts == Null,
                            If[Length[potentialmultipliedpartssolution[[1, \
                              finalindices[[isubin, 1, 1]], \
                              finalindices[[isubin, 1, 2]], \
                              finalindices[[isubin, 1, 3]]]]] == 2 && \
                              potentialmultipliedpartssolutionRank[[1, \
                              finalindices[[isubin, 1, 1]], \
                              finalindices[[isubin, 1, 2]], \
                              finalindices[[isubin, 1, 3]]]] == 0 && \
                              Head[potentialmultipliedpartssolution[[1, \
                              finalindices[[isubin, 1, 1]], \
                              finalindices[[isubin, 1, 2]], \
                              finalindices[[isubin, 1, 3]]]]] == Head[ \
                              dotdenoters[[1]]],
                                potentialmultipliedpartssolution[[1, \
                                  finalindices[[isubin, 1, 1]], \
                                  finalindices[[isubin, 1, 2]], \
                                  finalindices[[isubin, 1, 3]]]] = OperatorApplier[ \
                                  potentialmultipliedpartssolution[[1, \
                                  finalindices[[isubin, 1, 1]], \
                                  finalindices[[isubin, 1, 2]], \
                                  finalindices[[isubin, 1, 3]], 2]], dotdenoters, \
                                  potentialmultipliedpartssolution[[1, \
                                  finalindices[[isubin, 1, 1]], \
                                  finalindices[[isubin, 1, 2]], \
                                  finalindices[[isubin, 1, 3]], 1]]];
                                undottedparts = Undotter[potentialmultipliedpartssolution[[1, \
                                  finalindices[[isubin, 1, 1]], \
                                  finalindices[[isubin, 1, 2]], \
                                  finalindices[[isubin, 1, 3]]]], \
                            ...
                            If[undottedparts == Null,
                                Print["CRITICAL ERROR: Undotter: Unable to undot from the term. Term is: ", term];
                                Return[term];
                            ];
                        ];
                        
                        solution[[i, sectiontoplace]] = Expand[solution[[i, sectiontoplace]] + \
                          potentialmultipliedparts[[1, finalindices[[isubin, 1, 1]], \
                          finalindices[[isubin, 1, 2]], \
                          finalindices[[isubin, 1, 3]]]] * undottedparts];
                    ];
                    
                    If[savedlookforinfoRank == 2,
                        chosen = 1;
                        undottedparts = Undotter[potentialmultipliedpartssolution[[1, \
                          finalindices[[isubin, 1, 1]], \
                          finalindices[[isubin, 1, 2]], \
                            ...
                            If[Length[potentialmultipliedpartssolution[[1, \
                              ...
                              doubledotdenoters[[1]]],
                                potentialmultipliedpartssolution[[1, \
                                  ...
                                  finalindices[[isubin, 1, 1]], \
                                  finalindices[[isubin, 1, 2]], \
                                  finalindices[[isubin, 1, 3]], 2]], doubledotdenoters, \
                                  potentialmultipliedpartssolution[[1, \
                                  finalindices[[isubin, 1, 1]], \
                                  finalindices[[isubin, 1, 2]], \
                                  finalindices[[isubin, 1, 3]], 1]]];
                    ...
                    If[chosen == 0,
                        Print["CRITICAL ERROR: caseListmain: case not written yet. Term is: ", potentialmultipliedpartssolution];
                        Return[Null];
                    ];
                ];
                ...
                solution[[i]] = main[term[[i]], undotterms, scalars, vectors, tensors, \
                  closurevar, graddenoters, divdenoters, avgdenoters, derivativedenoters, dotdenoters, \
                  tensorproductdenoters, doubledotdenoters, pietrzykflipdenoters, divform, gradform, identitymatrixform, normalvecform, \
                  Uform, slowIV, fastIV, 0, option2];
...
Undotter[term_, option2_, dotdenoters_, doubledotdenoters_, identitymatrixform_] :=
    Block[{dimlesscoef, i, chosen = 0, newdot, undotted = 0, startnumber, chosen2 = 0, \
    dummytermpart, option2inpart, newterm},
        
        chosen2 = 0;
        
        If[Head[term] == Head[dotdenoters[[1]]] && chosen2 == 0,
            undotted = 0;
            startnumber = -1;
            option2inpart = -1;
            For[i = 1, i <= Length[term], i++,
                dummytermpart = term[[i]] /. option2 -> option2^2;
                If[term[[i]] == dummytermpart,
                    ,
                    If[term[[i]] == option2,
                        If[i == 1,
                            startnumber = 2;
            ...
            If[startnumber > -1,
                If[option2inpart == Length[term],
                    chosen2 = 1;
                    For[i = startnumber, i <= Length[term], i++,
                        chosen = 0;
                        If[i == startnumber,
                            chosen = 1;
                            newdot = term[[i]];
                        ];
                        If[term[[i]] == option2 && chosen == 0 && undotted == 0,
                            chosen = 1;
                            undotted = 1;
                        ];
                        If[chosen == 0,
                            newdot = OperatorApplier[newdot, dotdenoters, term[[i]]];
                        ];
                    ];
                ...
                chosen2 = 1;
                newdot = Undotter[term[[option2inpart]], option2, dotdenoters, \
                  doubledotdenoters, identitymatrixform];
                newterm = term;
                newterm[[option2inpart]] = newdot;
                dimlesscoef = newterm;
                
            ];
        ];
        
        If[Head[term] == Head[doubledotdenoters[[1]]] && chosen2 == 0,
            undotted = 0;
            startnumber = -1;
            option2inpart = -1;
            For[i = 1, i <= Length[term], i++,
            ...
            If[startnumber > -1,
                If[option2inpart == Length[term],
                    ...
                    For[i = startnumber, i <= Length[term], i++,
                        ...
                        If[chosen == 0,
                            newdot = OperatorApplier[newdot, doubledotdenoters, term[[i]]];
                        ];
                    ];
...
caseNonListmain[term_, lookfor_, scalars_, vectors_, tensors_, closurevar_, \
graddenoters_, divdenoters_, avgdenoters_, derivativedenoters_, dotdenoters_, tensorproductdenoters_, doubledotdenoters_, pietrzykflipdenoters_, divform_, \
gradform_, identitymatrixform_, normalvecform_, Uform_, slowIV_, fastIV_, option_, \
option2_] :=
...
gradform_, normalvecform_, Uform_, slowIV_, fastIV_, levelcountdummy_, option_] :=
    Block[{dimlesscoef, i, totalterm, Nterms, allowedterms, newterm, ii, Nnewterms},
        
        Nterms = Length[term[[1, 1]]];
        
        newterm = term;
        For[i = 1, i <= Nterms, i++,
            If[Head[term[[1, 1, i]]] == Power,
                If[term[[1, 1, i, 2]] == pietrzykflipdenoters[[2]],
                    ,
                    If[term[[1, 1, i, 2]] >= 0,
                        newterm[[1, 1, i]] = newterm[[1, 1, i, 1]];
                        ,
                        newterm[[1, 1, i]] = 1/newterm[[1, 1, i, 1]];
                    ];
                    For[ii = 2, ii <= Abs[term[[1, 1, i, 2]]], ii++,
                        newterm[[1, 1]] = Join[newterm[[1, 1]], {newterm[[1, 1, i]]}];
                        newterm[[1, 2]] = Join[newterm[[1, 2]], {newterm[[1, 2, i]]}];
                        newterm[[1, 3]] = Join[newterm[[1, 3]], {newterm[[1, 3, i]]}];
                    ];
                    ,
                    If[term[[1, 1, i, 2]] >= 0,
                        newterm[[1, 1, i]] = newterm[[1, 1, i, 1]];
        ...
        Nnewterms = Length[newterm[[1, 1]]];
        
        For[i = 1, i <= Nnewterms, i++,
            allowedterms = i;
            
            dimlesscoef = checkcaseTimes1[{{{}, {}, {}}}, isdependenton, scalars, vectors, tensors, \
              ...
              {0, allowedterms, newterm}];
            
            If[dimlesscoef == newterm,
                ,
                Break[];
                ,
                Break[];
...
graddenoters_, divdenoters_, avgdenoters_, derivativedenoters_, dotdenoters_, tensorproductdenoters_, doubledotdenoters_, pietrzykflipdenoters_, divform_, \
gradform_, normalvecform_, Uform_, slowIV_, fastIV_, levelcountdummy_, option_] :=
    Block[{dimlesscoef = {{{-1}}}, i, iprevious, totallength, passedterm, inow, testterm, totalterm, \
    multiplier, iremove, icheck, flagredoinow = 0, ipassed},
        ...
        If[Length[passedterm[[1, 1]]] == option[[2]],
        ...
        For[i = inow, i <= Length[totalterm[[1, 1]]], i++,
            ...
            dimlesscoef = checkcaseTimes1[passedterm, isdependenton, scalars, vectors, tensors, \
              closurevar, graddenoters, divdenoters, avgdenoters, derivativedenoters, dotdenoters, \
              tensorproductdenoters, doubledotdenoters, pietrzykflipdenoters, divform, gradform, normalvecform, Uform, slowIV, fastIV, \
              levelcountdummy, {i, option[[2]], totalterm}];
...
graddenoters_, divdenoters_, avgdenoters_, derivativedenoters_, dotdenoters_, tensorproductdenoters_, doubledotdenoters_, pietrzykflipdenoters_, divform_, gradform_, normalvecform_, \
Uform_, slowIV_, fastIV_, levelcountdummy_, option_] :=
    Block[{dimlesscoef = {{{term}, {0}, {0}}}, i, flagsubin, flaginteger, subinindex, \
    integerindex, dimlesscoef2, integer, chosen},
        Return[dimlesscoef];
    ];
...
Uform_, slowIV_, fastIV_, levelcountdummy_, option_] :=
    ...
    dummy = ConstantArray[0, Length[isdependenton]], scalvecten, subinindex, flagsubin},
        ...
        If[looking == 1,
            ...
            If[flagsubin == 0,
                ...
                dimlesscoef = checkcaseTimes[dimlesscoef, isdependenton, scalars, vectors, \
                  tensors, closurevar, graddenoters, divdenoters, avgdenoters, derivativedenoters, dotdenoters, \
                  tensorproductdenoters, doubledotdenoters, pietrzykflipdenoters, divform, gradform, normalvecform, Uform, slowIV, fastIV, \
                ...
                If[subinindex == 0,
                    ...
                    dimlesscoef[[1, 1]] = Join[dimlesscoef[[1, 1]], {-1}];
                    dimlesscoef[[1, 2]] = Join[dimlesscoef[[1, 2]], {0}];
                    dimlesscoef[[1, 3]] = Join[dimlesscoef[[1, 3]], {0}];
                    ...
                    If[subinindex == 0,
                        dimlesscoef[[1, 1]] = Delete[dimlesscoef[[1, 1]], -1];
                        dimlesscoef[[1, 2]] = Delete[dimlesscoef[[1, 2]], -1];
                        dimlesscoef[[1, 3]] = Delete[dimlesscoef[[1, 3]], -1];
                        ,
                        dimlesscoef[[1, 1]] = Join[dimlesscoef[[1, 1]], {-1}];
                        dimlesscoef[[1, 2]] = Join[dimlesscoef[[1, 2]], {0}];
                        dimlesscoef[[1, 3]] = Join[dimlesscoef[[1, 3]], {0}];
...
levelcountdummy_, option_] :=
    ...
    flaginteger = 0, integerindex = 0, chosen = 0, integer, ii, dropindices},
