...
BeginPackage["Disruptioneering`SlavaForm`"];
...
SlavaForm[term_, scalars_, vectors_, tensors_, closurevar_, graddenoters_, divdenoters_, \
...
option_] := 
    procedure[term, scalars, vectors, tensors, closurevar, graddenoters, \
...
procedure[term_, scalars_, vectors_, tensors_, closurevar_, graddenoters_, \
...
divform_, gradform_, normalvecform_, Uform_, slowIV_, fastIV_, option_] :=
    ...
    avgoverIV, correspondingsplitdomains = {}, \
    avgdenoterssplitdomains = avgdenoters[[2, ;;, 3]]},
        
        newvectors = vectors;
        
        solution = main[Expand[term], scalars, newvectors, tensors, closurevar, \
...
main[term_, scalars_, vectors_, tensors_, closurevar_, graddenoters_, divdenoters_, \
...
normalvecform_, Uform_, slowIV_, fastIV_, option_, option2_, option3_] :=
    Block[{solution},
        If[Head[term] == List,
            solution = caseListmain[term, scalars, vectors, tensors, closurevar, \
              graddenoters, divdenoters, avgdenoters, derivativedenoters, dotdenoters, tensorproductdenoters, \
              doubledotdenoters, pietrzykflipdenoters, divform, gradform, normalvecform, Uform, slowIV, fastIV, \
              option, option2, option3];
            ,
            solution = caseNonListmain[term, scalars, vectors, tensors, \
...
caseListmain[term_, scalars_, vectors_, tensors_, closurevar_, graddenoters_, \
...
slowIV_, fastIV_, option_, option2_, option3_] :=
    Block[{i, solution = ConstantArray[0, Length[term]]},
        For[i = 1, i <= Length[term], i++,
            solution[[i]] = main[term[[i]], scalars, vectors, tensors, \
              closurevar, graddenoters, divdenoters, avgdenoters, derivativedenoters, dotdenoters, tensorproductdenoters, doubledotdenoters, pietrzykflipdenoters, divform, \
              gradform, normalvecform, Uform, slowIV, fastIV, option, option2, option3];
        ];
...
caseNonListmain[term_, scalars_, vectors_, tensors_, closurevar_, \
...
Uform_, slowIV_, fastIV_, option_, option2_, option3_] :=
    Block[{i, solution, solutionvec},
        ...
        If[Head[term] == Plus,
            ...
            For[i = 1, i <= Length[term], i++,
                solutionvec[[i]] = casePlusSubstituter[term[[i]], scalars, \
            ...
            ,
            solution = casePlusSubstituter[term, scalars, vectors, tensors, \
              closurevar, graddenoters, divdenoters, avgdenoters, derivativedenoters, dotdenoters, tensorproductdenoters, doubledotdenoters, pietrzykflipdenoters, divform, \
              gradform, normalvecform, Uform, slowIV, fastIV, option, option2, option3];
            ,
            solution = casePlusSubstituter[term, scalars, vectors, tensors, \
...
casePlusSubstituter[term_, scalars_, vectors_, tensors_, closurevar_, \
...
gradform_, normalvecform_, Uform_, slowIV_, fastIV_, option_, option2_, option3_] :=
    ...
    correspondingsplitdomain},
        
        solution = getterminfo[term, fastIV, scalars, vectors, tensors, closurevar, \
...
(*This is implemented in getterminfo*)
undoNewOperator[term_, isdependenton_, scalars_, vectors_, tensors_, closurevar_, graddenoters_, \
divdenoters_, avgdenoters_, derivativedenoters_, dotdenoters_, tensorproductdenoters_, doubledotdenoters_, pietrzykflipdenoters_, \
divform_, gradform_, normalvecform_, Uform_, slowIV_, fastIV_, levelcountdummy_, option_] :=
    Block[{dimlesscoef, argument = term[[1, 1]], \
    avgoverIV = {term[[1, 2, 1]], term[[2, 1]]}},
        
        dimlesscoef = getterminfo[argument, isdependenton, scalars, vectors, tensors, closurevar, \
        ...
        If[isdependenton == avgoverIV,
            dimlesscoef[[1, 3, 1]] = 0;
        ];
        
        dimlesscoef = Inactive[Integrate][Inactive[Integrate][dimlesscoef[[1, 1, 1]], \
          {avgoverIV[[1]], 0, 1}], {avgoverIV[[2]], 0, 1}];
...
(*This is implemented in getterminfo*)
applyNewOperator[term_, isdependenton_, scalars_, vectors_, tensors_, closurevar_, graddenoters_, \
divdenoters_, avgdenoters_, derivativedenoters_, dotdenoters_, tensorproductdenoters_, doubledotdenoters_, pietrzykflipdenoters_, \
divform_, gradform_, normalvecform_, Uform_, slowIV_, fastIV_, levelcountdummy_, option_] :=
    Block[{dimlesscoef = term, subscript = term[[1, 1, 1, 2]], \
    argument = term[[1, 1, 1, 1, 1]], flag, avgdenoterssymbols = avgdenoters[[2, ;;, 1]], \
    avgdenotersIV = avgdenoters[[2, ;;, 2]], flagavgoverIV, avgoverIV, \
    avgdenoterssplitdomainsymbols = avgdenoters[[2, ;;, 3]], avgoversymbol, i},
        ...
        For[i = 1, i <= Length[avgdenoterssymbols], i++,
            If[Intersection[{subscript}, avgdenoterssymbols[[i]]] != {} || \
              Intersection[{subscript}, Flatten[avgdenoterssplitdomainsymbols[[i]]]] != {},
                ...
                avgoversymbol = subscript;
                Break[];
            ];
        ];
        If[flagavgoverIV == 0,
            Print["CRITICAL ERROR: applyNewOperator: Unidentified average in variable avgover. Variable is: ", avgover];
            Return[Null];
        ];
        
        If[avgdenoters[[2, 1, 1, 1]] == subscript,
            dimlesscoef[[1, 1, 1]] = Inactive[Integrate][Inactive[Integrate][argument, \
              {avgoverIV[[1]], 0, 1}], {avgoverIV[[2]], 0, 1}];
        ];
    ...
    dependencies, i, flagsubscript = 0},
        ...
        If[levelcount == 50,
            ...
            If[Head[term] == Head[avgdenoters[[1, 1]]] && chosen == 0,
                ...
                dimlesscoef = MakeSingleTerm[dimlesscoef, {}, dotdenoters, \
                  tensorproductdenoters, doubledotdenoters];
                dimlesscoef = applyNewOperator[dimlesscoef, isdependenton, scalars, \
                  vectors, tensors, closurevar, graddenoters, divdenoters, avgdenoters, \
            ...
            If[Head[term] == Inactive[Integrate] && chosen == 0,
                If[printouttoggle == 1,
                    Print["getterminfo: Identified Inactive Integral"];
                ];
                chosen = 1;
                dimlesscoef = undoNewOperator[term, isdependenton, scalars, vectors, \
                  tensors, closurevar, graddenoters, divdenoters, avgdenoters, derivativedenoters, dotdenoters, \
                  tensorproductdenoters, doubledotdenoters, pietrzykflipdenoters, divform, gradform, normalvecform, \
                  Uform, slowIV, fastIV, levelcountdummy, option];
            ];
