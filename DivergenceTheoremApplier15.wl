...
(****** DivergenceTheoremApplier.wl -- Given a second order differential equation, this function averages
once in terms of the fast variables. When it finds a divergence operator, it turns the integral
into a surface integral using the Divergence Theorem.
*************)

(* 

DivergenceTheoremApplier[term, ] 
...
BeginPackage["Disruptioneering`DivergenceTheoremApplier`"];


DivergenceTheoremApplier::usage = 
"DivergenceTheoremApplier[term, ]  Given a second order differential equation, this function averages
once in terms of the fast variables. When it finds a divergence operator, it turns the integral
into a surface integral using the Divergence Theorem.";
...
DivergenceTheoremApplier[term_, avgover_, scalars_, vectors_, tensors_, closurevar_, \
...
divdenoters_, avgdenoters_, derivativedenoters_, dotdenoters_, tensorproductdenoters_, doubledotdenoters_, pietrzykflipdenoters_, divform_, \
gradform_, normalvecform_, Uform_, slowIV_, fastIV_, option_] :=
    Block[{solution, uvecdependency, newvectors, i, ii, flagavgoverIV = 0, \
    avgdenoterssymbols = avgdenoters[[2, ;;, 1]], avgdenotersIV = avgdenoters[[2, ;;, 2]], \
    avgoverIV, correspondingavgoversymbol},
        
        (*Combine closure variables and normal vector into vectors.*)
        newvectors = vectors;
                
        (*Find IV for symbol you are using divergence theorem on*)
        flagavgoverIV = 0;
        For[i = 1, i <= Length[avgdenoterssymbols], i++,
            If[Intersection[avgover, avgdenoterssymbols[[i]]] != {},
                ...
                For[ii = 1, ii <= Length[avgdenoterssymbols[[i]]], ii++,
                    If[Intersection[avgover, {avgdenoterssymbols[[i, ii]]}] == {},
                        correspondingavgoversymbol = {avgdenoterssymbols[[i, ii]]};
                    ];
                ];
                If[correspondingavgoversymbol == 0,
                    correspondingavgoversymbol = ToExpression[ \
                      ToString[avgover[[1]]] <> ToString[avgover[[1]]]];
                    Print["Warning: procedure: No boundary symbol found. Using the following in boundary cases if needed: ", correspondingavgoversymbol];
                ];
        ...
        If[flagavgoverIV == 0,
            Print["CRITICAL ERROR: procedure: Unidentified average in variable avgover. Variable is: ", avgover];
            Return[Null];
        ];
...
main[term_, avgover_, scalars_, vectors_, tensors_, closurevar_, graddenoters_, divdenoters_, \
...
slowIV_, fastIV_, option_, option2_, option3_] :=
    Block[{i, solution = ConstantArray[0, Length[term]]},
        
        For[i = 1, i <= Length[term], i++,
            solution[[i]] = main[term[[i]], avgover, scalars, vectors, tensors, \
              ...
              gradform, normalvecform, Uform, slowIV, fastIV, option, option2, option3];
        ];
...
caseNonListmain[term_, avgover_, scalars_, vectors_, tensors_, closurevar_, \
...
Uform_, slowIV_, fastIV_, option_, option2_, option3_] :=
...
gradform_, normalvecform_, Uform_, slowIV_, fastIV_, option_, option2_, option3_] :=
    Block[{solution, levelcountdummy = 0, i},
        
        avgoverIVglobal = option2;
        avgoversymbolglobal = avgover;
        correspondingavgoversymbolglobal = option3;
        
        solution = getterminfo[term, option2, scalars, vectors, tensors, closurevar, \
...
(*This is implemented in subsubcaseAverageSimp*)
applyDivergenceTheorem[term_, subscript_, IVofsubscript_, isdependenton_, scalars_, vectors_, \
...
slowIV_, fastIV_, levelcountdummy_, option_] :=
    Block[{dimlesscoef = term, i, ii, summedterm, chosen = 0, flag1 = 0, dependencies, \
    argument, termargument, derivativeIndices, chosen2 = 0, argumentparts, gotone, divarg, \
    multarg, newterm},
        
        For[i = 1, i <= Length[term], i++,
            chosen = 0;
            summedterm = listProduct[term[[i, 1]]];
            
            (*If the argument to the average is a divergence*)
            If[Head[summedterm] == Head[divdenoters[[1]]],
                chosen = 1;
                
                (*Check Divergence argument to see which term is going to be dotted with the 
                normal vector.*)
                argumentparts = getterminfo[summedterm[[1]], summedterm[[2]], scalars, vectors, tensors, \
                ...
                If[Length[argumentparts] > 1,
                    newterm = MakeSingleTerm[argumentparts, {}, dotdenoters, \
                      tensorproductdenoters, doubledotdenoters];
                    dimlesscoef = applyDivergenceTheorem[newterm, subscript, IVofsubscript, \
                ...
                gotone = 0;
                multarg = 1;
                For[ii = 1, ii <= Length[argumentparts[[1, 2]]], ii++,
                    If[argumentparts[[1, 2, ii]] > 0,
                        gotone += 1;
                        divarg = argumentparts[[1, 1, ii]];
                    ];
                    If[argumentparts[[1, 2, ii]] == 0,
                        multarg *= argumentparts[[1, 1, ii]];
                    ];
                ];
                If[gotone != 1,
                    Print["CRITICAL ERROR: applyDivergenceTheorem: More than one term with > 0 tensor ranking in divergence, or a scalar. Ambiguous which term should be dotted with normal, or if this should happen. Term is: ", term];
                    Return[Null];
                ];
                
                dimlesscoef[[i, 1]] = {-1, correspondingavgoversymbolglobal[[1]], \
                  avgoversymbolglobal[[1]]^-1, OperatorApplier[OperatorApplier[normalvecform, \
                  dotdenoters, divarg] * multarg, avgdenoters[[1]], \
                  correspondingavgoversymbolglobal[[1]]]};
                
                dimlesscoef[[i, 2]] = {0, 0, 0, Total[term[[i, 2]]]};
                
                dimlesscoef[[i, 3]] = {0, 0, 0, 0};
            ];
            
            (*If the argument to the average is a derivative*)
            If[Head[summedterm] == Head[derivativedenoters[[1]]],
                chosen = 1;
                
                argument = summedterm[[1]];
                
                dimlesscoef[[i, 1]] = {-1, correspondingavgoversymbolglobal[[1]], \
                  avgoversymbolglobal[[1]]^-1, OperatorApplier[argument, avgdenoters[[1]], \
                  correspondingavgoversymbolglobal[[1]]]};
            ...
            If[chosen == 0,
                (*Log the term*)
                dimlesscoef[[i, 1]] = {OperatorApplier[listProduct[term[[i, 1]]], \
                  avgdenoters[[1]], subscript]};
                
                (*Adjust scalar, vector, or tensor indication*)
                dimlesscoef[[i, 2]] = {Total[term[[i, 2]]]};
...
(*This is implemented in subsubcaseDoubleDot*)
subsubcaseDoubleDotSimp[term_, isdependenton_, scalars_, vectors_, tensors_, closurevar_, \
graddenoters_, divdenoters_, avgdenoters_, derivativedenoters_, dotdenoters_, tensorproductdenoters_, \
...
levelcountdummy_, option_] :=
    ...
    doubledot, dummy, pietrzykargument, gradientargument, gradientIV, newterm2, \
    newdivergenceargument, newavgargument, newterm, pietrzykargumentRank, gradientargumentRank, \
    otherargument, otherargumentRank},
        
        For[iterms = 1, iterms <= Length[term], iterms++,
            chosen = 0;
            
            If[Intersection[avgoverIVglobal, isdependenton] != {},
                If[Length[term[[iterms, 1]]] == 1 && chosen == 0,
                    If[Head[term[[iterms, 1, 1]]] == List,
                        If[Length[term[[iterms, 1, 1]]] == 2,
                            If[Head[term[[iterms, 1, 1, 1]]] == Head[pietrzykflipdenoters[[1]]] && \
                              term[[iterms, 1, 1, 1, 2]] == pietrzykflipdenoters[[2]] && \
                              term[[iterms, 3, 1, 1]] == 0,
                                pietrzykargument = term[[iterms, 1, 1, 1, 1]];
                                pietrzykargumentRank = term[[iterms, 2, 1, 1]];
                                If[Head[term[[iterms, 1, 1, 2]]] == Head[avgdenoters[[1, 1]]] && \
                                  term[[iterms, 1, 1, 2, 2]] == avgoversymbolglobal[[1]],
                                    If[Head[term[[iterms, 1, 1, 2, 1, 1]]] == \
                                      Head[graddenoters[[1]]] && \
                                      term[[iterms, 1, 1, 2, 1, 1, 2]] == avgoverIVglobal,
                                        chosen = 1;
                                        
                                        gradientIV = term[[iterms, 1, 1, 2, 1, 1, 2]];
                                        gradientargument = term[[iterms, 1, 1, 2, 1, 1, 1]];
                                        gradientargumentRank = term[[iterms, 2, 1, 2]] - 1;
                                        
                                        If[gradientargumentRank == 1,
                                            newdivergenceargument = OperatorApplier[ \
                                              pietrzykargument, dotdenoters, gradientargument];
                                        ];
                                        If[gradientargumentRank > 1,
                                            newdivergenceargument = OperatorApplier[ \
                                              pietrzykargument, doubledotdenoters, gradientargument];
                                        ];
                                        If[gradientargumentRank == 0,
                                            Print["CRITICAL ERROR: subsubcaseDoubleDotSimp: The gradient argument is a scalar. Case not written yet."];
                                            Return[Null];
                                        ];
                                        
                                        newavgargument = OperatorApplier[ \
                                          newdivergenceargument, divdenoters, gradientIV];
                                        newterm = {{{newavgargument}, \
                                          {Abs[pietrzykargumentRank - gradientargumentRank] - 1}, \
                                          {1}}};
                                        
                                        newterm2 = applyDivergenceTheorem[newterm, \
                                          avgoversymbolglobal[[1]], avgoverIVglobal, \
                                          isdependenton, scalars, vectors, tensors, closurevar, \
                                          graddenoters, divdenoters, avgdenoters, \
                                          derivativedenoters, dotdenoters, \
                                          tensorproductdenoters, doubledotdenoters, \
                                          pietrzykflipdenoters, divform, gradform, \
                                          normalvecform, Uform, slowIV, fastIV, \
                                          levelcountdummy, option];
                                        
                                        newargumentparts2 = Join[newargumentparts2, newterm2];
                                    ];
                                ];
                            ];
                            ...
                            If[Head[term[[iterms, 1, 1, 1]]] == Head[pietrzykflipdenoters[[1]]] && \
                              term[[iterms, 1, 1, 1, 2]] == pietrzykflipdenoters[[2]] && \
                              term[[iterms, 3, 1, 1]] == 0 && chosen == 0,
                                
                                pietrzykargument = term[[iterms, 1, 1, 1, 1]];
                                pietrzykargumentRank = term[[iterms, 2, 1, 1]];
                                
                                otherargument = term[[iterms, 1, 1, 2]];
                                otherargumentRank = term[[iterms, 2, 1, 2]];
                                
                                If[Head[pietrzykargument] == Head[avgdenoters[[1, 1]]] && \
                                  pietrzykargument[[2]] == avgoversymbolglobal[[1]],
                                    If[Head[pietrzykargument[[1, 1]]] == \
                                      Head[graddenoters[[1]]] && \
                                      pietrzykargument[[1, 1, 2]] == avgoverIVglobal,
                                        chosen = 1;
                                        
                                        gradientIV = pietrzykargument[[1, 1, 2]];
                                        gradientargument = pietrzykargument[[1, 1, 1]];
                                        gradientargumentRank = pietrzykargumentRank - 1;
                                        
                                        If[gradientargumentRank == 1,
                                            newdivergenceargument = OperatorApplier[ \
                                              otherargument, dotdenoters, gradientargument];
                                        ];
                                        If[gradientargumentRank > 1,
                                            newdivergenceargument = OperatorApplier[ \
                                              otherargument, doubledotdenoters, gradientargument];
                                        ];
                                        If[gradientargumentRank == 0,
                                            Print["CRITICAL ERROR: subsubcaseDoubleDotSimp: The gradient argument is a scalar. Case not written yet."];
                                            Return[Null];
                                        ...
                                        newavgargument = OperatorApplier[ \
                                          newdivergenceargument, divdenoters, gradientIV];
                                        newterm = {{{newavgargument}, \
                                          {Abs[otherargumentRank - gradientargumentRank] - 1}, \
                                          {1}}};
...
levelcountdummy_, option_] :=
    ...
    camebackSum = 0, distributeddoubledot, expandedterm, doubledotexpandedterm, chosen3 = 0},
        ...
        If[camebackSum == 1,
            ...
            dimlesscoef = subsubcaseDoubleDotSimp[dimlesscoef, isdependenton, scalars, vectors, \
              tensors, closurevar, graddenoters, divdenoters, avgdenoters, derivativedenoters, dotdenoters, \
              tensorproductdenoters, doubledotdenoters, pietrzykflipdenoters, divform, gradform, normalvecform, \
              Uform, slowIV, fastIV, levelcountdummy, option];
...
slowIV_, fastIV_, levelcountdummy_, option_] :=
    ...
    vector, termdotunfold, chosen2 = 0, chosen3 = 0, term2, termdotunfold2},
        ...
        For[iterms = 1, iterms <= Length[term], iterms++,
            ...
            (*If the argument of the gradient is a double dot product*)
            If[Length[term[[iterms, 1]]] == 1 && chosen == 0,
                If[Head[term[[iterms, 1, 1]]] == Head[doubledotdenoters[[1]]],
    ...
    dummynewargumentparts2, iterms, imult, icombine},
        ...
        If[Intersection[IVofsubscript, isdependenton] != {},
            ...
            If[Length[newargumentparts[[1, 1]]] > 0,
                ...
                If[Intersection[avgoversymbolglobal, {subscript}] != {},
                    chosen2 = 1;
                    newargumentparts = applyDivergenceTheorem[newargumentparts, subscript, \
...
slowIV_, fastIV_, levelcountdummy_, option_] :=
    ...
    termnotdot, termdot, newtermnotdot, flagnotdot},
        ...
        For[iterms = 1, iterms <= Length[term], iterms++,
            ...
            (*If the argument of the average is a double dot product.*)
            If[Length[term[[iterms, 1]]] == 1 && chosen == 0,
                If[Head[term[[iterms, 1, 1]]] == Head[doubledotdenoters[[1]]],
