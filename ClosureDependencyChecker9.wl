...
(****** ClosureDependencyChecker.wl -- Check if the Closure Problems depend on the large scale variables
*************)

(* 

ClosureDependencyChecker[term, ] 
...
BeginPackage["Disruptioneering`ClosureDependencyChecker`"];

ClosureDependencyChecker::usage = 
"ClosureDependencyChecker[term, ] ";
...
ClosureDependencyChecker[term_, isdependenton_, scalars_, vectors_, tensors_, closurevar_, \
...
gradform_, normalvecform_, Uform_, slowIV_, fastIV_, option_] := 
    procedure[term, isdependenton, scalars, vectors, tensors, closurevar, graddenoters, \
...
procedure[term_, isdependenton_, scalars_, vectors_, tensors_, closurevar_, graddenoters_, \
...
normalvecform_, Uform_, slowIV_, fastIV_, option_] :=
    Block[{solution, uvecdependency, newvectors, i, solutiondependencies},
        
        newvectors = vectors;
        If[closurevar != {},
            For[i = 1, i <= Length[closurevar], i++,
                newvectors = Join[newvectors, {dependencyadder[closurevar[[i]], fastIV]}];
            ];
        ];
        newvectors = Join[newvectors, {normalvecform}];
        
        {solution, solutiondependencies} = main[Expand[term], isdependenton, scalars, \
          newvectors, tensors, closurevar, graddenoters, divdenoters, avgdenoters, derivativedenoters, dotdenoters, \
        ...
        Return[{solution, solutiondependencies}];
    ];


main[term_, isdependenton_, scalars_, vectors_, tensors_, closurevar_, graddenoters_, \
divdenoters_, avgdenoters_, derivativedenoters_, dotdenoters_, tensorproductdenoters_, doubledotdenoters_, pietrzykflipdenoters_, divform_, gradform_, \
normalvecform_, Uform_, slowIV_, fastIV_, option_] :=
    Block[{solution, solutiondependencies},
        
        If[Head[term] == List,
            {solution, solutiondependencies} = caseListmain[term, isdependenton, scalars, \
            ...
            ,
            {solution, solutiondependencies} = caseNonListmain[term, isdependenton, scalars, \
              vectors, tensors, closurevar, graddenoters, divdenoters, avgdenoters, derivativedenoters, \
              dotdenoters, tensorproductdenoters, doubledotdenoters, pietrzykflipdenoters, divform, gradform, normalvecform, Uform, \
              slowIV, fastIV, option];
            ,
...
caseListmain[term_, isdependenton_, scalars_, vectors_, tensors_, closurevar_, graddenoters_, \
divdenoters_, avgdenoters_, derivativedenoters_, dotdenoters_, tensorproductdenoters_, doubledotdenoters_, pietrzykflipdenoters_, divform_, gradform_, \
normalvecform_, Uform_, slowIV_, fastIV_, option_] :=
    Block[{i, solution = ConstantArray[0, Length[term]], \
    solutiondependencies = ConstantArray[0, Length[term]]},
        
        For[i = 1, i <= Length[term], i++,
            {solution[[i]], solutiondependencies[[i]]} = main[term[[i]], isdependenton, \
              scalars, vectors, tensors, closurevar, graddenoters, divdenoters, avgdenoters, derivativedenoters, \
              dotdenoters, tensorproductdenoters, doubledotdenoters, pietrzykflipdenoters, divform, gradform, normalvecform, Uform, \
              slowIV, fastIV, option];
        ];
        
        solutiondependencies = Total[solutiondependencies];
        
        If[solutiondependencies > 1,
...
caseNonListmain[term_, isdependenton_, scalars_, vectors_, tensors_, closurevar_, \
graddenoters_, divdenoters_, avgdenoters_, derivativedenoters_, dotdenoters_, tensorproductdenoters_, doubledotdenoters_, pietrzykflipdenoters_, divform_, \
gradform_, normalvecform_, Uform_, slowIV_, fastIV_, option_] :=
    Block[{i, solution, solutionvec, solutiondependencies},
                
        If[Head[term] == Plus,
            solutionvec = ConstantArray[0, Length[term]];
            solutiondependencies = ConstantArray[0, Length[term]];
            For[i = 1, i <= Length[term], i++,
                {solutionvec[[i]], solutiondependencies[[i]]} = casePlusSubstituter[ \
                  term[[i]], isdependenton, scalars, vectors, tensors, closurevar, \
            ...
            solution = {Total[solutionvec], Total[solutiondependencies]};
            
            If[solution[[2]] > 1,
                solution[[2]] = 1;
            ];
            ,
            solution = casePlusSubstituter[term, isdependenton, scalars, vectors, tensors, \
...
casePlusSubstituter[term_, isdependenton_, scalars_, vectors_, tensors_, closurevar_, \
graddenoters_, divdenoters_, avgdenoters_, derivativedenoters_, dotdenoters_, tensorproductdenoters_, doubledotdenoters_, pietrzykflipdenoters_, divform_, \
gradform_, normalvecform_, Uform_, slowIV_, fastIV_, option_] :=
    Block[{solution, levelcountdummy = 0, i},
        
        solution = getterminfo[term, isdependenton, scalars, vectors, tensors, closurevar, \
        ...
        Return[{solution[[1, 1, 1]], solution[[1, 3, 1]]}];
    ];
