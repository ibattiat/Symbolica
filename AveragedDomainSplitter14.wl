...
(****** AveragedDomainSplitter.wl -- 
*************)

(* 

AveragedDomainSplitter[term, ] 
...
BeginPackage["Disruptioneering`AveragedDomainSplitter`"];

AveragedDomainSplitter::usage = 
...
AveragedDomainSplitter[term_, avgover_, scalars_, vectors_, tensors_, closurevar_, \
...
normalvecform_, newnormalvecform_, Uform_, slowIV_, fastIV_, option_] := 
    procedure[term, avgover, scalars, vectors, tensors, closurevar, graddenoters, divdenoters, \
      avgdenoters, derivativedenoters, dotdenoters, tensorproductdenoters, doubledotdenoters, \
      pietrzykflipdenoters, divform, gradform, normalvecform, newnormalvecform, Uform, \
...
doubledotdenoters_, pietrzykflipdenoters_, divform_, gradform_, normalvecform_, \
newnormalvecform_, Uform_, slowIV_, fastIV_, option_] :=
    ...
    avgoverIV, correspondingsplitdomains = {}, correspondingsplitdomainsnormalvectors = {}, \
    avgdenoterssplitdomains = avgdenoters[[2, ;;, 3]], correspondingdomainnormalvector = {}},
        
        (*Combine closure variables and normal vector into vectors.*)
        newvectors = vectors;
        
        (*Find IV for symbol of the domain you are splitting*)
        flagavgoverIV = 0;
        For[i = 1, i <= Length[avgdenoterssymbols], i++,
            If[Intersection[avgover, avgdenoterssymbols[[i]]] != {},
                flagavgoverIV = 1;
                avgoverIV = avgdenotersIV[[i]];
                correspondingdomainnormalvector = normalvecform[[i]];
                correspondingsplitdomainsnormalvectors = newnormalvecform[[i]];
                
                For[ii = 1, ii <= Length[avgdenoterssymbols[[i]]], ii++,
                    If[Intersection[avgover, {avgdenoterssymbols[[i, ii]]}] != {},
                        correspondingsplitdomains = avgdenoterssplitdomains[[i, ii]];
                        Break[];
                    ];
                ];
                If[correspondingsplitdomains == {},
                    Print["Warning: No domains to split with the provided avgover symbol."];
                ];
        ...
        solution = main[Expand[term], avgover, scalars, newvectors, tensors, closurevar, \
          ...
          normalvecform, Uform, slowIV, fastIV, {}, avgoverIV, correspondingsplitdomains, \
          correspondingsplitdomainsnormalvectors, correspondingdomainnormalvector];
...
normalvecform_, Uform_, slowIV_, fastIV_, option_, option2_, option3_, option4_, option5_] :=
    Block[{solution},
        If[Head[term] == List,
            ...
            ,
            solution = caseNonListmain[term, avgover, scalars, vectors, tensors, \
...
slowIV_, fastIV_, option_, option2_, option3_, option4_, option5_] :=
...
divdenoters_, avgdenoters_, derivativedenoters_, dotdenoters_, tensorproductdenoters_, \
doubledotdenoters_, pietrzykflipdenoters_, divform_, gradform_, normalvecform_, Uform_, \
slowIV_, fastIV_, option_, option2_, option3_, option4_, option5_] :=
...
tensorproductdenoters_, doubledotdenoters_, pietrzykflipdenoters_, divform_, gradform_, \
normalvecform_, Uform_, slowIV_, fastIV_, option_, option2_, option3_, option4_, option5_] :=
    Block[{solution, levelcountdummy = 0, i, avgoverIVglobal, avgoversymbolglobal, \
    correspondingsplitdomains, correspondingsplitdomainsnormalvectors, \
    correspondingdomainnormalvector},
        
        avgoverIVglobal = option2;
        avgoversymbolglobal = avgover;
        correspondingsplitdomains = option3;
        correspondingsplitdomainsnormalvectors = option4;
        correspondingdomainnormalvector = option5;
...
applyDomainSplit[term_, subscript_, IVofsubscript_, isdependenton_, scalars_, vectors_, \
...
slowIV_, fastIV_, levelcountdummy_, option_] :=
    Block[{dimlesscoef, i, ii, iii, summedterm, chosen = 0, flag1 = 0, dependencies, \
    argument, termargument, derivativeIndices, chosen2 = 0, argumentparts, gotone, divarg, \
    multarg, newterm, inorm},
        ...
        If[Length[term] == 1,
            If[Head[term[[1, 1, 1]]] == Head[dotdenoters[[1]]],
                flagnormpresent = 0;
                For[inorm = 1, inorm <= Length[term[[1, 1, 1]]], inorm++,
                    If[term[[1, 1, 1, inorm]] == correspondingdomainnormalvector,
                        flagnormpresent = 1;
                        Break[];
                    ];
                ];
                If[flagnormpresent == 1,
                    For[i = 1, i <= Length[correspondingsplitdomains], i++,
                        newterm = term /. {correspondingdomainnormalvector -> \
                          correspondingsplitdomainsnormalvectors[[i]]};
                        For[ii = 1, ii <= Length[newterm], ii++,
                            newterm[[ii, 1]] = {correspondingsplitdomains[[i]], subscript^-1, \
                              OperatorApplier[listProduct[newterm[[ii, 1]]], avgdenoters[[1]], \
                              correspondingsplitdomains[[i]]]};
                            newterm[[ii, 2]] = {0, 0, Total[newterm[[ii, 2]]]};
                            newterm[[ii, 3]] = {0, 0, 0};
                        ];
                        dimlesscoef = Join[dimlesscoef, newterm];
                    ];
                ];
            ];
        ];
        ...
        If[Intersection[IVofsubscript, isdependenton] != {},
            ...
            If[Length[newargumentparts[[1, 1]]] > 0,
                ...
                If[Intersection[avgoversymbolglobal, {subscript}] != {},
                    chosen2 = 1;
                    newargumentparts = applyDomainSplit[newargumentparts, subscript, \
